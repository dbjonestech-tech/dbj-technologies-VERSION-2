...
**Wiring status as of May 3, 2026 (post-Commit-B):**

- **Now wired** (Commits A and B): `inngest-health-hourly`, `funnel-refresh-hourly`, `email-kpi-refresh-hourly` (Commit A, verified green in Inngest dashboard with multiple successful runs at 0% failure rate); `vercel-telemetry-hourly` (Commit B, this commit). The Commit A trio writes only to internal Postgres tables / materialized views. Commit B's `vercel-telemetry-hourly` calls the Vercel REST API once per hour at `:10` past, snapshotting deployments into the `vercel_deployments` table for the operator-visible rollup on `/admin/platform`.
- **Still unwired** (pending Commits C-E): `anthropic-budget-hourly`, `search-console-daily`, `infrastructure-check-daily`. All required env vars are confirmed set in Vercel production (`ANTHROPIC_ADMIN_KEY`, `ANTHROPIC_MONTHLY_BUDGET_USD`, `GOOGLE_SC_CREDENTIALS_JSON`, `GOOGLE_SC_SITE_URL`); registration unblocks per the staged risk-tier plan. The most operationally severe of these is `infrastructure-check-daily` (sole TLS/WHOIS/DNS-auth expiry warning channel, currently silent).
- Operational impact today: `/admin/infrastructure`, the budget tile on `/admin/costs`, and `/admin/search` remain stale until Commits C-E land. `/admin/platform` will hydrate Vercel deployment data at the next `:10` UTC fire of `vercel-telemetry-hourly` (within 60 minutes of this deploy).
- Staged plan: Commit C = Anthropic budget; D = Search Console; E = Infrastructure. Each commit deploys, then we wait for the cron's first scheduled fire (or manual Inngest invoke for the daily ones), then we verify success in the Inngest dashboard before the next ships. If a cron fails on first run, fix before moving forward.
