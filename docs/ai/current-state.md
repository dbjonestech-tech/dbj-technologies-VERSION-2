...
**Wiring status as of May 3, 2026 (post-Commit-D):**

- **Now wired** (Commits A, B, C, D): `inngest-health-hourly`, `funnel-refresh-hourly`, `email-kpi-refresh-hourly` (Commit A, verified green in Inngest dashboard); `vercel-telemetry-hourly` (Commit B, registered and visible); `anthropic-budget-hourly` (Commit C, defensively coded with local-event fallback); `search-console-daily` (Commit D, this commit, fires at 06:00 UTC daily). The `importSearchConsoleDaily` helper pulls trailing 7 days of impressions/clicks per page+query+country+device using the googleapis SDK, with ON CONFLICT updates handling late-arriving GSC data (which routinely lags 2-3 days).
- **Still unwired** (pending Commit E): `infrastructure-check-daily`. No new env vars required. Most operationally severe of the unwired list, since it is the sole TLS/WHOIS/DNS-auth expiry warning channel.
- Operational impact today: `/admin/infrastructure` remains stale until Commit E lands. Other admin dashboards (`/admin/costs` budget tile, `/admin/platform`, `/admin/search`) will hydrate at their respective crons' next scheduled fires.
- Commit D first-run caveat: the function depends on a GSC permissions UI grant for the service account associated with `GOOGLE_SC_CREDENTIALS_JSON`. This grant has not been verified. If the cron fails on its first 06:00 UTC fire with a 403, the fix is to grant the service account access in the GSC property's Users/Permissions page (no code change required); then manually invoke the function from the Inngest dashboard to backfill.
- Staged plan: Commit E = Infrastructure. Final commit in the staged registration plan.
