...
**Wiring status as of May 3, 2026 (post-Commit-C):**

- **Now wired** (Commits A, B, C): `inngest-health-hourly`, `funnel-refresh-hourly`, `email-kpi-refresh-hourly` (Commit A, verified green in Inngest dashboard with multiple successful runs at 0% failure rate); `vercel-telemetry-hourly` (Commit B, registered and visible in dashboard, awaiting first scheduled `:10` fire); `anthropic-budget-hourly` (Commit C, this commit, fires at `:20` past each hour). The `snapshotAnthropicBudget` helper is defensively coded: if the Anthropic Admin API call fails or returns no spend value, it falls back to `fetchLocalSpend()` against the local `api_usage_events` table, and the Postgres write is wrapped in try/catch. The function returns `ok: true` regardless of admin-API outcome, with a `source` field discriminating `admin_api` vs `local_events` for downstream observability.
- **Still unwired** (pending Commits D and E): `search-console-daily`, `infrastructure-check-daily`. Required env vars (`GOOGLE_SC_CREDENTIALS_JSON`, `GOOGLE_SC_SITE_URL`) confirmed set in Vercel production. The most operationally severe of the two is `infrastructure-check-daily` (sole TLS/WHOIS/DNS-auth expiry warning channel, currently silent).
- Operational impact today: `/admin/infrastructure` and `/admin/search` remain stale until Commits D and E land. The budget tile on `/admin/costs` will hydrate at the next `:20` UTC fire of `anthropic-budget-hourly` (within 60 minutes of this deploy). `/admin/platform` will hydrate Vercel deployment data at the next `:10` UTC fire of `vercel-telemetry-hourly`.
- Staged plan: Commit D = Search Console; E = Infrastructure. Each commit deploys, then we verify success in the Inngest dashboard. Commit D's first run depends on a GSC permissions UI grant for the service account that has not been verified; if the cron fails on its first daily run with a 403, the fix is granting the service account access in the GSC property's Users/Permissions page (no code change required).
