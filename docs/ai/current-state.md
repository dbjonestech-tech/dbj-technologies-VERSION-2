...
- 24 pages live, deployed on Vercel via GitHub `dbj-technologies-VERSION-2` (origin main)
...
- About page: dark hero section, split layout with joshua-jones.png (transparent PNG), clip-path reveal animation, floating geometric accents, character-level headline animation, story sections ("Why I Work This Way", "What You Actually Get", "How I Build", "Who This Is For"), personal sign-off, gradient divider, values cards with hover glow
...
- Pricing: Starter $4,500, Professional $9,500, Enterprise custom from $15,000. Maintenance $299/month
...
- Curated vertical database (5376a56 + 2afa3c9): 206 entries at lib/data/verticals.ts. Three-layer matching: exact name -> alias table (17 entries) -> fuzzy scoring with synonym expansion. 51 high/medium confidence verticals skip API call entirely.
...
- "Search Visibility" pillar (renamed from "Findability")
...
- Vercel env vars: RESEND_API_KEY, CONTACT_EMAIL, CONTACT_FROM_EMAIL, DATABASE_URL, ANTHROPIC_API_KEY, BROWSERLESS_TOKEN, INNGEST keys, Turnstile keys. SMTP_* vars deleted (migrated to Resend).
