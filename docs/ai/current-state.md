...
**Wiring status as of May 3, 2026 (post-Commit-A):**

- **Now wired** (Commit A, this commit): `inngest-health-hourly`, `funnel-refresh-hourly`, `email-kpi-refresh-hourly`. All three have zero env-var dependencies and write only to internal Postgres tables / materialized views. Empty-state UX in dependent admin pages was already verified by commit `7e6e159` so registration carries no regression risk.
- **Still unwired** (pending Commits B-E): `vercel-telemetry-hourly`, `anthropic-budget-hourly`, `search-console-daily`, `infrastructure-check-daily`. All four required env vars are confirmed set in Vercel production (`VERCEL_API_TOKEN`, `VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID`, `ANTHROPIC_ADMIN_KEY`, `ANTHROPIC_MONTHLY_BUDGET_USD`, `GOOGLE_SC_CREDENTIALS_JSON`, `GOOGLE_SC_SITE_URL`); registration unblocks per the staged risk-tier plan. The most operationally severe of these is `infrastructure-check-daily` (sole TLS/WHOIS/DNS-auth expiry warning channel, currently silent).
- Operational impact today: `/admin/platform`, `/admin/infrastructure`, the budget tile on `/admin/costs`, `/admin/search`, parts of the operator-visible Vercel cron rollup remain stale until Commits B-E land. `/admin/funnel`, the KPI rollup on `/admin/email`, and the catch-up Inngest self-monitoring on `/admin/pipeline` will hydrate at the next scheduled fire of the just-registered crons (within 60 minutes of this deploy).
- Staged plan: Commit B = Vercel telemetry; C = Anthropic budget; D = Search Console; E = Infrastructure. Each commit deploys, then we wait for the cron's first scheduled fire (or manual Inngest invoke for the daily ones), then we verify success in the Inngest dashboard before the next ships. If a cron fails on first run, fix before moving forward.
