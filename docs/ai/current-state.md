...
- About page: dark hero section, split layout with joshua-jones.webp (true alpha-channel transparent PNG sourced through cwebp -q 90 -alpha_q 100; subject silhouette floats over the dark page bg, with the existing radial-gradient ellipse mask in AboutContent.tsx now serving as a soft atmospheric fade rather than background-removal), clip-path reveal animation, floating geometric accents, character-level headline animation, story sections ("Why I Work This Way", "What You Actually Get", "How I Build", "Who This Is For"), personal sign-off, gradient divider, values cards with hover glow
