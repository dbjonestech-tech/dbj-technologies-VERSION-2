...
The Inngest pipeline steps execute in a specific order with data dependencies. Do NOT change the step execution order. The actual step.run sequence in lib/inngest/functions.ts is: validate-url -> capture-screenshots -> mark-analyzing -> run-audit (PSI/Lighthouse) -> ai-vision-audit -> ai-remediation -> research-benchmark -> ai-revenue-impact -> calculate-score -> finalize -> send-report-email -> send-followup-1 (48h sleep) -> send-followup-2 (5d sleep) -> send-breakup (8d sleep). Note that ai-remediation runs BEFORE research-benchmark and ai-revenue-impact — remediation does not depend on benchmark/revenue data.
