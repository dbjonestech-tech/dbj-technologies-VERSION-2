...
- [ ] Investigate `axios` high-severity advisories GHSA-3w6x-2g7m-8v23 (Invisible JSON Response Tampering via Prototype Pollution Gadget in `parseReviver`) and GHSA-q8qp-cvcw-x6jj (prototype pollution read-side gadgets enabling credential injection / request hijacking). Affects axios 1.0.0 - 1.15.1. Pre-existing transitive vuln, not introduced by the F2 next bump (verified via git stash test on 2026-05-04, axios was already in the lockfile pre-bump; the GHSA database entries were published between the security audit and the F2 patch). Action: identify which top-level dependency pulls axios in (`npm ls axios`), check whether `npm audit fix` resolves without breaking changes, ship as a separate dep-bump commit.
