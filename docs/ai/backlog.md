...
- [ ] Bump `eslint-config-next` from `^15.5.15` to a 16.x major to match the Next runtime. Pre-existing mismatch surfaced during the 2026-05-04 security patch bump (next 16.2.2 -> 16.2.4); deferred from that commit per scope-creep rule. Standalone task so any new lint rules have room to land.
...
- [ ] Rename `middleware.ts` to `proxy.ts`. Next 16 emits a deprecation warning during build (`The "middleware" file convention is deprecated. Please use "proxy" instead.` -- https://nextjs.org/docs/messages/middleware-to-proxy). Functional no-op rename that eliminates the build warning and aligns with the new Next file convention. Cosmetic priority.
