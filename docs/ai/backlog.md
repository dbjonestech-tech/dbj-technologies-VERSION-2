...
- [x] Bump `eslint-config-next` from `^15.5.15` to a 16.x major to match the Next runtime. DONE 2026-05-04. Chained eslint 8 -> 9 + .eslintrc.json -> eslint.config.mjs flat-config migration. eslint-config-next 16.x ships its own flat preset (`eslint-config-next/core-web-vitals`) so no FlatCompat bridge is needed.
- [ ] Audit `react-hooks` v7 / React Compiler diagnostics. The F4 eslint-config-next 16 bump turned on a new family of rules (`set-state-in-effect`, `static-components`, `refs`, `immutability`, `error-boundaries`, `purity`, `preserve-manual-memoization`) that surfaced 20+ violations across the codebase. Most are the documented `setMounted(true)` SSR-safety pattern. The three named real refactor opportunities are partly closed: `ServicePageLayout.tsx` Icon-created-in-render fixed via a stable `<ServiceIcon />` dispatcher (covers both the hero badge and the benefits map); `TaskRowClient.tsx` impure `Date.now()` during render fixed via `useState(() => Date.now())` lazy init capturing render-time-now once at mount. `AskPathlight.tsx` memoization issue NOT fixed: the diagnostic surfaces at `closePanel:177` but the actual root cause is `sendMessage`'s async closure complexity (mutates `abortRef`, conditional state writes, branching control flow). The "Compilation Skipped" warning applies to the whole component, so localized closePanel fixes don't move the diagnostic. Properly clearing it requires restructuring `sendMessage` (probably extracting the stream loop to a hook and aborting via a separate effect on `isOpen`). Out of scope for the partial walk; left for the broader rule-family pass. Whole family stays disabled in `eslint.config.mjs` until the rest of the violations are walked.
- [x] Investigate `axios` high-severity advisories GHSA-3w6x-2g7m-8v23 + GHSA-q8qp-cvcw-x6jj. DONE 2026-05-04. `npm ls axios` traced the only parent to `wait-on@9.0.5` (devDependency). Verified zero references to `wait-on` in scripts, CI workflows, lighthouserc, or source. Removed `wait-on` entirely; both advisories cleared and total vuln count dropped 11 -> 10 with 0 high.
- [x] Rename `middleware.ts` to `proxy.ts`. DONE 2026-05-04 (commit aafea4e). Functional no-op; updated 7 cross-file comment references from `middleware.ts` to `proxy.ts` in the same commit. Build warning cleared.
- [ ] Bump GitHub Actions to current majors. Workflow `.github/workflows/lighthouse-gate.yml` is now SHA-pinned at `actions/checkout` v4.3.1, `actions/setup-node` v4.4.0, `actions/upload-artifact` v4.6.2 (commit de717a9, 2026-05-05). Newer majors are available: checkout v6.0.2, setup-node v6.4.0, upload-artifact v7.0.1. Each may bring breaking changes (new minimum Node, deprecated input names, artifact format differences); upgrade one at a time and verify the workflow still passes. Renew the SHA pin when each bump lands.
