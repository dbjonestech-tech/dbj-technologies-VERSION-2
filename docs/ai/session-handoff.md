...
3. Screenshot the three portfolio templates at desktop width (`pi-law.html`, `luxury-builders.html`, `dental-practice.html`) and wire the screenshots into `lib/work-data.ts` as new case studies on the Work page. More templates may be coming, so stage the screenshot batch for the full set.
