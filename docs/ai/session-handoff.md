...
`main` is at `5e7e86e` (fix(work): stack metric/surface tiles on mobile so long labels stop overflowing), confirmed pushed to `origin main`. Working tree clean. Recent chain (most recent first): `5e7e86e` (mobile tile stack) -> `ccc5ab8` (snapshot for d7ee5cc) -> `d7ee5cc` (work-card previews 3:2) -> `8b3ce15` (snapshot for 0450119) -> `0450119` (Selected Builds + mobile work-page polish) -> `cd4d555` (snapshot for f14ccc5) -> `f14ccc5` (Admin Login text link in navbar) -> `299409a` (snapshot for d45df5d) -> `d45df5d` (middleware prefix-match fix) -> `14343d2` (snapshot for 7353a41) -> `7353a41` (auth surfaces split) -> `6ade672` (snapshot for 5fd29b7) -> `5fd29b7` (Design Briefs rename + 8 template previews + project-card layout) -> `2c714ec` (snapshot for 3a19312) -> `3a19312` (navbar dual auth surfaces) -> `5fbf385` (snapshot for 3d4c2fe) -> `3d4c2fe` (Client Portal rename + two-track /signin) -> `483d42f` (snapshot for c97ba70) -> `c97ba70` (GA + consent banner + privacy disclosure)

### Why this followup was needed

The middleware (`middleware.ts`) used `pathname.startsWith("/portal")` to gate access to `/portal/*`. That match also caught `/portal-access`, treating the public client-entry page as a protected portal route and redirecting unauthenticated visitors to `/signin?callbackUrl=/portal-access`. So the navbar correctly pointed at `/portal-access`, the page existed and rendered correctly server-side, but the middleware intercepted the request before the page could ever respond. Joshy hit this immediately on the first deploy: clicked the "Client Portal" pill, landed on the "Studio admin" page, was confused. The fix introduces `isPathOrSubpath()` which only matches exact paths or paths with a `/` separator, so `/portal-access` is no longer swallowed by the `/portal` rule. While in there, the unauthenticated redirect destinations are also now audience-aware: admin paths bounce to `/signin`, portal paths bounce to `/portal-access`. -> `3275b3d` (snapshot for 9775d0f) -> `9775d0f` (portal v1 + role split) -> `ef1e373` (snapshot for af176eb) -> `af176eb` (cross-template differentiation + Blueprints grid) -> `7723bd4` (snapshot for fd84067) -> `fd84067` (Stage 5 admin users + invitations) -> `30aeb6d` (snapshot for 551cd6f) -> `551cd6f` (templates image swap + hero polish) -> `f8808ef` (snapshot for 97051ca) -> `97051ca` (Stage 3 operational tools) -> `1ccc863` (snapshot for b1f59e4) -> `b1f59e4` (Stages 1+2 admin login portal) -> `8289370` (5 templates Pass 1 + blueprints) -> `b9b8dfe` (monitoring V1+V2) -> earlier chain elided in archive.
