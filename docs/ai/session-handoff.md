...
- **Portfolio template mockups for Work page** (064f89e). Two self-contained HTML files at `public/templates/pi-law.html` (Mercer & Associates, fictional Dallas PI law firm, dark navy + brass-gold authority design with Playfair Display + DM Sans) and `public/templates/luxury-builders.html` (Whitmore Custom Homes, fictional DFW custom home builder, warm white + sage editorial design with Cormorant Garamond + DM Sans). Pure HTML + hand-written CSS, zero JavaScript, noindex+nofollow meta, Google Fonts via `<link>`, unicode glyphs over Lucide for cross-browser monochrome rendering, DBJ credit linked in both footers. `robots.ts` disallow extended again to include `/templates/` so the mockups are blocked at the robots level (mockups are reference pieces for portfolio screenshots, not real businesses). Voice intentional: Mercer uses "we" (fictional firm), Whitmore uses "I" (mirrors DBJ positioning). These will be screenshotted for the Work page; they are NOT yet wired into `lib/work-data.ts`. Final disallow array: `["/api/", "/monitoring", "/pathlight/", "/templates/"]`.
...
3. Screenshot `/templates/pi-law.html` and `/templates/luxury-builders.html` at desktop width and wire them into `lib/work-data.ts` as new case studies on the Work page.
...
`main` is at `064f89e` (feat: portfolio template mockups for PI law firm and luxury custom builder), pushed to `origin main`. Working tree clean after this snapshot follow-up commit. Today's chain: `b5e1105` (history archive + compact handoff) → `5a97a26` (snapshot) → `433fb82` (service detail pages + pricing subtitle) → `4bc2bf9` (snapshot) → `d1d2a1f` (schema + robots) → `a89d139` (snapshot) → `064f89e` (template mockups).
