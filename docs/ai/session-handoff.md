...
`main` is at `b9b8dfe` (feat: in-house real-time monitoring (V1 + safe V2 enhancements)), confirmed pushed to `origin main`. Working tree clean (parallel-tab voice.ts Adam->Charlie swap landed in `ea39189` before this commit). Recent chain (most recent first): `b9b8dfe` (monitoring V1+V2) -> `de17ba0` (split moon rotation from libration for smoother motion) -> `ea39189` (ElevenLabs voice Adam -> Charlie) -> `c564eb9` (moon libration + aurora ribbons + pulsating rings) -> `ef11e35` (rate-limit PDF/chat + Pathlight moon loading state) -> `932109e` (snapshot for 7d17be5) -> `7d17be5` (CSP media-src + post-complete polling) -> `bab64ce` (snapshot for a68ce3e) -> `a68ce3e` (Resend webhook permissive schema + Blob notes) -> earlier chain elided in archive.
