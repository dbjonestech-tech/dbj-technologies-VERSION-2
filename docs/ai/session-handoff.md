...
- **Portfolio expanded to 8 perfected templates with locked design systems** (this commit). All eight portfolio mockups in `public/templates/` rebuilt or built from scratch off perfected prompt specs in `docs/template-prompts/`. New `docs/template-prompts/` directory holds 9 files: `README.md` (distinction matrix + universal CSS quality bar) and one prompt per template (`01-pi-law.md` through `08-restaurant.md`). Every prompt locks a CSS-variable design token block, an explicit type scale via `clamp()`, a unique signature design element, and a "must NOT resemble" guardrail block restating the other seven templates' tokens. The 8 templates and their distinct lanes:
...
`main` is at `c737f91` (feat: rebuild all 8 portfolio templates with perfected design systems), confirmed pushed to `origin main`. Working tree clean. Today's chain: `b5e1105` (history archive + compact handoff) → `5a97a26` (snapshot) → `433fb82` (service detail pages + pricing subtitle) → `4bc2bf9` (snapshot) → `d1d2a1f` (schema + robots) → `a89d139` (snapshot) → `064f89e` (first two templates) → `f2c9b29` (snapshot) → `add122b` (dental template) → `3c346c3` (snapshot) → `c737f91` (all 8 templates rebuilt to perfected specs + docs/template-prompts/).
