...
`main` is at `add122b` (feat: dental practice portfolio template), pushed to `origin main`. Working tree clean after this snapshot follow-up commit. Today's chain: `b5e1105` (history archive + compact handoff) → `5a97a26` (snapshot) → `433fb82` (service detail pages + pricing subtitle) → `4bc2bf9` (snapshot) → `d1d2a1f` (schema + robots) → `a89d139` (snapshot) → `064f89e` (first two templates) → `f2c9b29` (snapshot) → `add122b` (dental template).
